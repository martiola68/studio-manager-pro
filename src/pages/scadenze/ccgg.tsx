...
export default function ScadenzeCCGGPage() {
  ...
  const loadScadenze = async (): Promise<ScadenzaCCGG[]> => {
    const { data, error } = await supabase
      ...
      .select(`
        *,
        professionista:tbutenti!tbscadccgg_utente_professionista_id_fkey(nome, cognome),
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Gestione Certificazioni Conformi Generale e Globale</p>
        </div>
      </div>
