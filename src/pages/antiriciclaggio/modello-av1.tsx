              ...
              <div className="overflow-x-auto">
                <div className="min-w-[760px]">
                  <div className="grid grid-cols-5 gap-0 border border-gray-400">
                    ...
                    <div className={h-24 border border-gray-400 bg-orange-300 ${isActiveCell("molto", "abbastanza")}} />
                    ...
                    <div className={h-24 border border-gray-400 bg-yellow-200 ${isActiveCell("abbastanza", "non")}} />
                    <div className={h-24 border border-gray-400 bg-yellow-200 ${isActiveCell("abbastanza", "poco")}} />
                    <div className={h-24 border border-gray-400 bg-orange-300 ${isActiveCell("abbastanza", "abbastanza")}} />
                    <div className={h-24 border border-gray-400 bg-orange-300 ${isActiveCell("abbastanza", "molto")}} />
                    ...
                    <div className={h-24 border border-gray-400 bg-orange-300 ${isActiveCell("poco", "abbastanza")}} />
                    ...
                    <div className={h-24 border border-gray-400 bg-yellow-200 ${isActiveCell("non", "abbastanza")}} />
...
valori ammessi: 0, 1, 2, 3, 4

default: 0

Eliminare qualsiasi valore null, undefined o vuoto

Il modello deve partire sempre con tutti i valori a 0
...
Eventuali calcoli collegati (TotA, TotB, Media, ecc.)

2. Aggiornamento pesi rischio

Sostituire ovunque nel modello:
...
Qualsiasi formula o funzione che usa i pesi

⚠️ Vincoli importanti

NON modificare la struttura database esistente (salvo necessario)

NON rompere il salvataggio attuale (che ora funziona)

I valori devono restare persistenti (no reset dopo save)

🔍 Extra da verificare (se possibile)

Se esistono valori già salvati null → convertirli a 0

Validazione lato frontend per bloccare valori fuori range

Coerenza tra UI e calcoli finali

📍 Punto di partenza codice

src/pages/antiriciclaggio/modello-av1.tsx

Tabelle: public.tbAV1


Ragionato per 25s
Perfetto, riprendiamo da qui.
Ti lascio un intervento unico e coerente per src/pages/antiriciclaggio/modello-av1.tsx, senza toccare la struttura DB e senza rompere il salvataggio.

Cosa cambia
Vengono sistemati questi punti:

tutti i valori A1–A4 e B1–B6 partono da 0

valori ammessi solo: 0, 1, 2, 3, 4

qualunque null, undefined, stringa vuota o valore fuori range viene normalizzato a 0

pesi aggiornati da 40/60 a 30/70

persistenza invariata dopo il salvataggio

coerenza UI + calcoli + matrice finale

Modifiche da applicare
1) Aggiungi questi helper sopra export default function ModelloAV1Page()
Sotto toNumber puoi inserire questo blocco:
...
2) Sostituisci l’inizializzazione di formData
Adesso hai:
...
3) Sostituisci i calcoli TotA / TotB usando normalizeScore
Adesso hai:
...
const RischioEffettivo = Number((RisInerentePonderato + RisSpecificoPonderato).toFixed(2));
5) Correggi la logica della matrice attiva
Questa parte oggi usa i valori già ponderati:

const categoriaInerente = getCategoriaRischio(RisInerentePonderato);
const categoriaVulnerabilita = getCategoriaRischio(MediaPunteggio);
Con i nuovi pesi 30/70, usare il valore ponderato nella matrice rende il quadrante poco coerente.
Meglio usare:

rischio inerente = punteggio prestazione puro

vulnerabilità = media punteggio

Quindi sostituisci con:

const categoriaInerente = getCategoriaRischio(punteggioPrestazione);
const categoriaVulnerabilita = getCategoriaRischio(MediaPunteggio);
6) Quando carichi un record, converti tutto a 0 invece che 1
Dentro loadRecordById, adesso hai:
...
Quindi il blocco finale di setFormData dentro loadRecordById diventa così:
...
7) Aggiorna i select dei punteggi a 0–4
Adesso hai:
...
<select
  ...
  onChange={(e) =>
    setFormData((prev) => ({
      ...prev,
      [sectionKey]: normalizeScore(e.target.value),
    }))
  }
>
...
8) Normalizza il payload prima del save
Dentro handleSave, adesso nel payload hai i valori presi direttamente da formData:
...
Quindi il payload completo diventa:
...
9) Aggiorna le etichette della matrice
Nella UI della matrice sostituisci:

RISCHIO INERENTE 40% → RISCHIO INERENTE 30%

VULNERABILITÀ 60% → VULNERABILITÀ 70%
...
Versione completa delle parti logiche già corrette
Ti lascio il blocco centrale già pronto, così fai meno incolla-frammenti.

Helper + stato + calcoli
function toNumber(value: unknown) {
  const n = Number(value);
  return Number.isFinite(n) ? n : 0;
}
...
const categoriaInerente = getCategoriaRischio(punteggioPrestazione);
const categoriaVulnerabilita = getCategoriaRischio(MediaPunteggio);
Esito atteso dopo questa modifica
Dopo il fix:

nuovo AV1 → tutti i valori partono a 0

record esistente con campi null → viene caricato con 0

select accetta solo 0..4

salvataggio resta funzionante

nessun reset post-save

pesi coerenti 30/70

matrice finale coerente con i nuovi pesi

Nota importante sulla media
Ho lasciato MediaPunteggio = (TotA + TotB) / 10 invariata, perché è la media dei 10 valori A/B e non dipende dai pesi 30/70.
I pesi vanno applicati correttamente solo a:
...
intestazioni/logica della matrice

Questa è la scelta più corretta rispetto al codice che hai ora.

Se vuoi, nel prossimo messaggio ti preparo direttamente il file modello-av1.tsx completo già riscritto integralmente, pronto da incollare.


ma riscrivi tutto che facciamo prima...
