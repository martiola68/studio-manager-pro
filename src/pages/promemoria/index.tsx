...
export default function PromemoriaPage() {
 ...
 const [selectedPromemoria, setSelectedPromemoria] = useState<Promemoria | null>(null);
  ...
  // NUOVO: Stato per filtro operatore
  ...
  <div className="container mx-auto py-8 px-4">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-6">
        <div className="flex items-center space-x-2">
          ...
          />
          <Label htmlFor="vista-miei-promemoria" className="cursor-pointer">Visualizza i miei promemoria</Label>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          />
          <Label htmlFor="vista-promemoria-creati" className="cursor-pointer">Visualizza i promemoria che ho inviato</Label>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          />
          <Label htmlFor="vista-promemoria-tutti" className="cursor-pointer">Visualizza tutto</Label>
        </div>
        ...
        <div className="flex items-center space-x-2">
          <Checkbox
            ...
            onCheckedChange={(checked) => {
              ...
              if (isChecked) setFiltroVistaPromemoria("tutti");
            }}
          />
          <Label
            htmlFor="visualizza-tutti-settore"
            className={!isResponsabileSettore ? "cursor-not-allowed opacity-60 text-red-600" : "cursor-pointer text-red-600 font-medium"}
          >
...
.sort((a, b) => {
  ...
  return dataA - dataB;
            ...
            .map((p) => {
              ...
              return (
                <TableRow key={p.id} className={isOverdue ? "bg-red-50" : isCompleted ? "bg-green-50" : isAnnullata ? "bg-gray-100" : ""}>
                  ...
                  <TableCell>
                    <Badge variant="outline" className={p.priorita === "Alta" ? "border-red-500 text-red-700 bg-red-50" : p.priorita === "Media" ? "border-yellow-500 text-yellow-700 bg-yellow-50" : p.priorita === "Bassa" ? "border-green-500 text-green-700 bg-green-50" : ""}>
                      {p.priorita || "-"}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell>
                    <Badge
                      variant={p.working_progress === "Completato" ? "default" : p.working_progress === "In lavorazione" ? "secondary" : p.working_progress === "Aperto" ? "outline" : p.working_progress === "Presa visione" ? "outline" : p.working_progress === "Richiesta confronto" ? "secondary" : p.working_progress === "Annullata" ? "destructive" : "outline"}
                      className={p.working_progress === "Aperto" ? "border-blue-500 text-blue-700" : p.working_progress === "In lavorazione" ? "bg-yellow-100 text-yellow-800" : p.working_progress === "Completato" ? "bg-green-100 text-green-800" : p.working_progress === "Presa visione" ? "border-cyan-500 text-cyan-700 bg-cyan-50" : p.working_progress === "Richiesta confronto" ? "bg-purple-100 text-purple-800" : p.working_progress === "Annullata" ? "bg-red-100 text-red-800" : ""}
                    >
                      {p.working_progress}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell>
                    {Array.isArray(p.allegati) && p.allegati.length > 0 ? (
                      <div className="flex items-center gap-2">
                        <span className="flex items-center gap-1 text-gray-600"><Paperclip className="h-4 w-4" />{p.allegati.length}</span>
                        <Button variant="outline" size="sm" onClick={() => handleViewAllegati(p)} className="h-8 w-8 p-0" title="Visualizza allegati"><Eye className="h-4 w-4" /></Button>
                      </div>
                    ) : <span className="text-gray-400">-</span>}
                  </TableCell>
                  <TableCell className="text-center"><Checkbox checked={p.working_progress === "Completato"} onCheckedChange={(checked) => handleToggleCompletato(p, checked === true)} title="Completato" /></TableCell>
                  <TableCell className="text-center"><Button variant="ghost" size="icon" onClick={() => handleEdit(p)} title="Modifica"><Pencil className="h-4 w-4" /></Button></TableCell>
                </TableRow>
              );
            })}

          {promemoriaFiltrati.length === 0 && (
            <TableRow><TableCell colSpan={14} className="text-center py-8 text-gray-500">Nessun promemoria trovato</TableCell></TableRow>
          )}
        </TableBody>
      </Table>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleCreate} className="space-y-4 pt-4">
                ...
                <SelectContent>
                  {tipiPromemoria.map(t => (
                    <SelectItem key={t.id} value={t.id}>
                      <span className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full border" style={{ backgroundColor: t.colore || "#3B82F6" }} />{t.nome}</span>
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div>
              <Label>Titolo *</Label>
              <Input value={formData.titolo} onChange={e => setFormData(prev => ({...prev, titolo: e.target.value}))} required />
            </div>
            <div>
              <Label>Descrizione</Label>
              <Textarea value={formData.descrizione} onChange={e => setFormData(prev => ({...prev, descrizione: e.target.value}))} />
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>Data Inserimento</Label>
                <Input type="date" value={format(formData.data_inserimento, "yyyy-MM-dd")} onChange={(e) => {
                  if (!e.target.value) return;
                  setFormData((prev) => ({ ...prev, data_inserimento: new Date(`${e.target.value}T00:00:00`) }));
                }} />
              </div>
              <div>
                <Label>Giorni Scadenza</Label>
                <Input type="number" min="0" value={formData.giorni_scadenza} onChange={e => setFormData(prev => ({...prev, giorni_scadenza: parseInt(e.target.value) || 0}))} />
              </div>
            </div>

            <div>
              <Label>Data Scadenza (calcolata automaticamente)</Label>
              <Input type="date" value={format(formData.data_scadenza, "yyyy-MM-dd")} disabled className="bg-gray-100" />
            </div>
            ...
            <div className="flex items-center space-x-2 p-4 border rounded-lg bg-gray-50">
              <Checkbox id="invio-multiplo-create" checked={invioMultiplo} onCheckedChange={(checked) => {
                ...
                if (checked) setFormData(prev => ({ ...prev, destinatario_id: "", settore: "" }));
                else setFormData(prev => ({ ...prev, destinatari_multipli: [] }));
              }} />
            ...
            {!invioMultiplo ? (
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><Label>Settore</Label><Input value={formData.settore} disabled className="bg-gray-100" /></div>
              </div>
            ) : (
              <div className="space-y-4 border rounded-lg p-4">
                <Label>Destinatari Multipli</Label>
                <Input placeholder="Cerca destinatari..." value={searchDestinatari} onChange={(e) => setSearchDestinatari(e.target.value)} className="mb-2" />
                <div className="flex flex-wrap gap-2 mb-3">
                  <Button type="button" variant="outline" size="sm" onClick={handleSelezioneLavoro}>Settore Lavoro</Button>
                  <Button type="button" variant="outline" size="sm" onClick={handleSelezioneFiscale}>Settore Fiscale</Button>
                  ...
                  <Button type="button" variant="outline" size="sm" onClick={handleDeselezionaTutti}>Deseleziona Tutti</Button>
                </div>
                <div className="max-h-60 overflow-y-auto space-y-2 border rounded p-3 bg-white">
                  {utentiFiltrati.map((u) => (
                    <div key={u.id} className="flex items-center space-x-2">
                      <Checkbox id={`dest-${u.id}`} checked={formData.destinatari_multipli.includes(u.id)} onCheckedChange={() => toggleDestinatario(u.id)} />
                      <Label htmlFor={`dest-${u.id}`} className="cursor-pointer flex-1">{u.nome} {u.cognome} {u.settore ? `(${u.settore})` : ''}</Label>
                    </div>
                  ))}
                  {utentiFiltrati.length === 0 && <p className="text-sm text-gray-500 italic">Nessun utente trovato</p>}
                </div>
                <p className="text-sm text-gray-600">Selezionati: {formData.destinatari_multipli.length} utent{formData.destinatari_multipli.length === 1 ? 'e' : 'i'}</p>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select value={formData.priorita} onValueChange={v => setFormData(prev => ({...prev, priorita: v}))}>
                  ...
                  <SelectContent><SelectItem value="Bassa">Bassa</SelectItem><SelectItem value="Media">Media</SelectItem><SelectItem value="Alta">Alta</SelectItem></SelectContent>
                </Select>
              </div>
              <div>
                <Label>Stato</Label>
                <Select value={formData.working_progress} onValueChange={(val) => setFormData({...formData, working_progress: val})}>
                  ...
                  <SelectContent>
                    <SelectItem value="Aperto">Aperto</SelectItem><SelectItem value="In lavorazione">In lavorazione</SelectItem><SelectItem value="Completato">Completato</SelectItem><SelectItem value="Presa visione">Presa visione</SelectItem><SelectItem value="Richiesta confronto">Richiesta confronto</SelectItem><SelectItem value="Annullata">Annullata</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-2 py-2">
              <Checkbox id="invia-teams" checked={formData.invia_teams} onCheckedChange={(checked) => setFormData(prev => ({...prev, invia_teams: !!checked}))} />
              <div className="grid gap-1.5 leading-none"><Label htmlFor="invia-teams" className="flex items-center gap-1 cursor-pointer"><MessageSquare className="h-3.5 w-3.5 text-blue-600" />Invia notifica su Teams</Label></div>
            </div>

            <div>
              <Label className="flex items-center gap-2 mb-2"><Paperclip className="h-4 w-4" /> Allegati</Label>
                ...
                <div className="space-y-2 mt-2">
                  {filesToUpload.map((file, idx) => (
                    <div key={idx} className="flex justify-between items-center text-sm bg-white p-2 rounded border">
                      ...
                      <Button type="button" variant="ghost" size="sm" onClick={() => removeFileToUpload(idx)}><X className="h-4 w-4 text-red-500" /></Button>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleUpdate} className="space-y-4 pt-4">
            <div>
              <Label>Tipo Promemoria</Label>
              <Select value={formData.tipo_promemoria_id} onValueChange={v => setFormData(prev => ({...prev, tipo_promemoria_id: v}))} disabled={!!(currentUser && selectedPromemoria && currentUser.id === selectedPromemoria.destinatario_id && currentUser.id !== selectedPromemoria.operatore_id)}>
                <SelectTrigger><SelectValue placeholder="Seleziona tipo..." /></SelectTrigger>
                <SelectContent>
                  {tipiPromemoria.map(t => (
                    <SelectItem key={t.id} value={t.id}>
            ...
            <div>
              <Label>Titolo *</Label>
              <Input value={formData.titolo} onChange={e => setFormData(prev => ({...prev, titolo: e.target.value}))} required disabled={!!(currentUser && selectedPromemoria && currentUser.id === selectedPromemoria.destinatario_id && currentUser.id !== selectedPromemoria.operatore_id)} />
            </div>
            <div>
              <Label>Descrizione</Label>
              <Textarea value={formData.descrizione} onChange={e => setFormData(prev => ({...prev, descrizione: e.target.value}))} disabled={!!(currentUser && selectedPromemoria && currentUser.id === selectedPromemoria.destinatario_id && currentUser.id !== selectedPromemoria.operatore_id)} />
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>Data Inserimento</Label>
                <Input type="date" value={formData.data_inserimento ? format(formData.data_inserimento, "yyyy-MM-dd") : ""} onChange={e => setFormData(prev => ({...prev, data_inserimento: new Date(e.target.value)}))} disabled={!!(currentUser && selectedPromemoria && currentUser.id === selectedPromemoria.destinatario_id && currentUser.id !== selectedPromemoria.operatore_id)} />
              </div>
              <div>
                <Label>Giorni Scadenza</Label>
                <Input type="number" min="0" value={formData.giorni_scadenza} onChange={e => setFormData(prev => ({...prev, giorni_scadenza: parseInt(e.target.value) || 0}))} disabled={!!(currentUser && selectedPromemoria && currentUser.id === selectedPromemoria.destinatario_id && currentUser.id !== selectedPromemoria.operatore_id)} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>Destinatario</Label>
                <Select value={formData.destinatario_id || "none"} onValueChange={handleDestinatarioChange} disabled={!!(currentUser && selectedPromemoria && currentUser.id === selectedPromemoria.destinatario_id && currentUser.id !== selectedPromemoria.operatore_id)}>
                  <SelectTrigger><SelectValue placeholder="Seleziona..." /></SelectTrigger>
                  <SelectContent><SelectItem value="none">Nessuno</SelectItem>{utenti.map(u => <SelectItem key={u.id} value={u.id}>{u.nome} {u.cognome}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div><Label>Settore</Label><Input value={formData.settore} disabled className="bg-gray-100" /></div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>Priorità</Label>
                <Select value={formData.priorita} onValueChange={v => setFormData(prev => ({...prev, priorita: v}))} disabled={!!(currentUser && selectedPromemoria && currentUser.id === selectedPromemoria.destinatario_id && currentUser.id !== selectedPromemoria.operatore_id)}>
                  <SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Bassa">Bassa</SelectItem><SelectItem value="Media">Media</SelectItem><SelectItem value="Alta">Alta</SelectItem></SelectContent>
                </Select>
              </div>
              <div>
                <Label>Stato</Label>
                <Select value={formData.working_progress} onValueChange={(val) => setFormData({...formData, working_progress: val})}>
                  <SelectTrigger><SelectValue placeholder="Seleziona stato"/></SelectTrigger>
                  <SelectContent><SelectItem value="Aperto">Aperto</SelectItem><SelectItem value="In lavorazione">In lavorazione</SelectItem><SelectItem value="Completato">Completato</SelectItem><SelectItem value="Presa visione">Presa visione</SelectItem><SelectItem value="Richiesta confronto">Richiesta confronto</SelectItem><SelectItem value="Annullata">Annullata</SelectItem></SelectContent>
                </Select>
              </div>
            </div>

            <div className="flex items-center space-x-2 py-2">
              <Checkbox id="invia-teams-edit" checked={formData.invia_teams} onCheckedChange={(checked) => setFormData(prev => ({...prev, invia_teams: !!checked}))} />
              <div className="grid gap-1.5 leading-none"><Label htmlFor="invia-teams-edit" className="flex items-center gap-1 cursor-pointer"><MessageSquare className="h-3.5 w-3.5 text-blue-600" />Invia notifica su Teams</Label></div>
            </div>
                ...
                {selectedPromemoria?.allegati && (selectedPromemoria.allegati as unknown as Allegato[]).map((a, idx) => {
                  ...
                  return (
                    <div key={idx} className={`flex justify-between items-center p-2 rounded border ${isDeleted ? 'bg-red-50 opacity-50' : 'bg-white'}`}>
                      <div className="flex items-center gap-2 overflow-hidden">{getFileIcon(a.nome)}<span className={`text-sm truncate ${isDeleted ? 'line-through' : ''}`}>{a.nome}</span></div>
                      <div className="flex gap-1">
                        {!isDeleted ? <><Button type="button" variant="ghost" size="sm" onClick={() => handleOpenAllegato(a)}><Eye className="h-4 w-4" /></Button>{!isRecipientOnly && <Button type="button" variant="ghost" size="sm" onClick={() => markAttachmentForDeletion(a.url)}><Trash2 className="h-4 w-4 text-red-500" /></Button>}</> : !isRecipientOnly && <Button type="button" variant="ghost" size="sm" onClick={() => undoAttachmentDeletion(a.url)}>Ripristina</Button>}
                      </div>
                    </div>
                  );
                })}
                {(!selectedPromemoria?.allegati || (selectedPromemoria.allegati as unknown as Allegato[]).length === 0) && <p className="text-sm text-gray-400 italic">Nessun allegato presente</p>}
              </div>
            </div>
      ...
      <Dialog open={isAllegatiDialogOpen} onOpenChange={setIsAllegatiDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle className="flex items-center gap-2"><Paperclip className="h-5 w-5" />Allegati</DialogTitle></DialogHeader>
            ...
            {viewAllegatiPromemoria?.allegati && (viewAllegatiPromemoria.allegati as unknown as Allegato[]).length > 0 ? (
              (viewAllegatiPromemoria.allegati as unknown as Allegato[]).map((allegato, index) => (
                <div key={index} className="flex items-center justify-between p-3 border rounded-lg hover:bg-gray-50 transition-colors">
                  <div className="flex items-center gap-3 flex-1 overflow-hidden">{getFileIcon(allegato.nome)}<div className="flex-1 min-w-0"><p className="font-medium text-sm truncate">{allegato.nome}</p><p className="text-xs text-gray-500">{(allegato.size / 1024).toFixed(1)} KB</p></div></div>
                  <Button variant="secondary" size="sm" onClick={() => handleOpenAllegato(allegato)} className="ml-2 shrink-0 gap-1"><Eye className="h-4 w-4" />Apri</Button>
                </div>
              ))
            ) : <p className="text-center text-gray-500 italic">Nessun allegato</p>}
          </div>
          <DialogFooter><Button variant="outline" onClick={() => setIsAllegatiDialogOpen(false)}>Chiudi</Button></DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isDocViewerOpen} onOpenChange={setIsDocViewerOpen}>
        <DialogContent className="max-w-5xl h-[85vh] p-0 flex flex-col gap-0 overflow-hidden">
          <div className="flex items-center justify-between p-4 border-b bg-gray-50"><h3 className="font-semibold flex items-center gap-2"><FileText className="h-4 w-4" /> Anteprima Documento</h3><Button variant="ghost" size="icon" onClick={() => setIsDocViewerOpen(false)}><X className="h-4 w-4" /></Button></div>
          <iframe src={currentDocUrl} className="flex-1 w-full border-0 bg-gray-100" title="Document Viewer" />
        </DialogContent>
      </Dialog>

      <Dialog open={isConfrontoDialogOpen} onOpenChange={setIsConfrontoDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>Richiesta confronto</DialogTitle><DialogDescription>Inserisci la motivazione da inviare automaticamente via email al creatore del promemoria.</DialogDescription></DialogHeader>
          <div className="space-y-3 pt-2"><div><Label>Motivazione *</Label><Textarea value={motivazioneConfronto} onChange={(e) => setMotivazioneConfronto(e.target.value)} placeholder="Scrivi la motivazione della richiesta di confronto..." rows={5} /></div></div>
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => { setIsConfrontoDialogOpen(false); setMotivazioneConfronto(""); }}>Annulla</Button>
            <Button type="button" onClick={handleConfermaRichiestaConfronto} disabled={loading}>{loading ? "Invio..." : "Invia richiesta confronto"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Conferma Eliminazione</DialogTitle><DialogDescription>{selectedIds.length === 1 ? "Sei sicuro di voler eliminare questo promemoria?" : `Sei sicuro di voler eliminare ${selectedIds.length} promemoria selezionati?`} L'azione è irreversibile.</DialogDescription></DialogHeader>
          <DialogFooter><Button variant="outline" onClick={() => setIsDeleteDialogOpen(false)}>Annulla</Button><Button variant="destructive" onClick={handleBulkDeleteConfirm}>Elimina</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
