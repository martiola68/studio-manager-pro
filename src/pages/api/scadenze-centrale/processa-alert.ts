...
import { resolveAutomaticMicrosoftAlertSender } from "@/services/automaticMicrosoftAlertSender";
...
function calcolaTipoAlert(scadenza: ScadenzaRow, oggi: string) {
  ...
  if (giorniResidui === 0) {
    return {
      ...
      chiaveInvio: `scade-oggi-${oggi}`,
      oggetto: `Scadenza di oggi - ${scadenza.titolo}`,
      intestazione: "La scadenza è prevista per oggi",
    };
  }

  return {
    tipoAlert: "preavviso",
    chiaveInvio: `preavviso-${giorniResidui}-${oggi}`,
...
function prossimoLunediMattina(dataInput: string): string {
  ...
  // Il cron giornaliero parte alle 06:00 UTC: l'alert deve essere già esigibile.
  ...
  const preavvisi = Array.from(
    new Set(
      (scadenza.intervalli_alert?.length
        ...
        .filter((valore): valore is number => Number.isInteger(valore) && valore >= 0)
        .sort((a, b) => b - a)
    )
  );

  const dateProgrammate = preavvisi
    .map((giorni) => ({ giorni, data: sottraiGiorni(scadenza.data_scadenza, giorni) }))
    .filter((item) => item.data > oggi)
    .sort((a, b) => a.data.localeCompare(b.data));

  // Il processore viene eseguito alle 06:00 UTC: programmando a mezzanotte
  // evitiamo che il cron passi prima dell'orario dell'alert e lo rinvii al giorno dopo.
  if (dateProgrammate.length > 0) return `${dateProgrammate[0].data}T00:00:00.000Z`;
  if (giorniResidui > 0) return `${scadenza.data_scadenza}T00:00:00.000Z`;
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!SECRET || (querySecret !== SECRET && bearerSecret !== SECRET)) {
  ...
  try {
    ...
    let queryScadenze = supabaseAdmin
      ...
      .select(`id,studio_id,cliente_id,operatore_responsabile_id,origine_modulo,origine_tabella,origine_record_id,tipo_scadenza,titolo,metadati,descrizione,data_scadenza,stato,giorni_preavviso_1,giorni_preavviso_2,giorni_preavviso_3,intervalli_alert,prossimo_alert_at,numero_alert_inviati,link_dettaglio`)
      .eq("stato", "attiva")
      .not("prossimo_alert_at", "is", null)
      .lte("prossimo_alert_at", adesso)
      .order("prossimo_alert_at", { ascending: true })
      .limit(200);
    ...
    for (const riga of (scadenze || []) as ScadenzaRow[]) {
      ...
      if (isMemoCalendarioSenzaScadenzario(riga) && giorniResiduiMemo < 0) {
        const { error } = await supabaseAdmin
          .from("tbscadenze_centrale")
          .update({ prossimo_alert_at: null, updated_at: new Date().toISOString() })
          .eq("id", riga.id)
        ...
        dettagli.push({
          scadenza_id: riga.id,
          ok: true,
          messaggio: "Memo di calendario terminato al giorno zero",
        });
        continue;
      }
      ...
      if (!microsoftConnectionId) {
        ...
        dettagli.push({
          scadenza_id: riga.id,
          ok: false,
          messaggio: "Connessione Microsoft dello studio assente",
        });
        continue;
      }

      const { data: assegnazioni, error: assegnazioniError } = await supabaseAdmin
        .from("tbscadenze_centrale_destinatari")
        .select("utente_id,destinatario_email,tipo_destinatario")
        .eq("studio_id", riga.studio_id)
      ...
      const destinatariInterniIds = Array.from(
        new Set(
          (assegnazioni || [])
            .filter((a) => a.tipo_destinatario === "interno" && a.utente_id)
            .map((a) => String(a.utente_id))
        )
      );

      const destinatariEsterni = Array.from(
        new Set(
          (assegnazioni || [])
            .filter((a) => a.tipo_destinatario === "esterno" && a.destinatario_email)
            .map((a) => String(a.destinatario_email).trim().toLowerCase())
            .filter(Boolean)
        )
      );

      if (
        !destinatariInterniIds.length &&
        !destinatariEsterni.length &&
        riga.operatore_responsabile_id
      ) {
        destinatariInterniIds.push(riga.operatore_responsabile_id);
      }
      ...
      const destinatari = [
        ...destinatariInterni.map((d) => ({
          tipo: "interno" as const,
          utente_id: String(d.id),
          nome: d.nome,
          cognome: d.cognome,
          email: String(d.email),
        })),
        ...destinatariEsterni.map((email) => ({
      ...
      if (!destinatari.length && riga.tipo_scadenza !== "documento_aml") {
        saltati++;
        dettagli.push({
          scadenza_id: riga.id,
          ok: false,
          messaggio: "Nessun destinatario valido associato alla scadenza",
        });
        continue;
      }

      const { data: tokenDisponibili, error: tokenError } = await supabaseAdmin
      ...
      const mittenteAlternativoId =
        automaticSender?.senderUserId ||
        utentiConTokenIds.find((id) => mittentiValidiSet.has(id)) ||
        null;
      ...
      if (riga.tipo_scadenza === "documento_aml") {
        const { data: documentoAml, error: documentoAmlError } = await supabaseAdmin
          .from("tbclienti_documenti_aml")
          .select("id,studio_id,soggetto_cliente_id,public_doc_enabled,public_doc_token,documento_richiesto_il")
          .eq("id", riga.origine_record_id)
          .eq("studio_id", riga.studio_id)
          .eq("attivo", true)
          .maybeSingle();
        ...
        if (documentoAmlError || !documentoAml?.id) {
          ...
          dettagli.push({
            scadenza_id: riga.id,
            ok: false,
            messaggio: documentoAmlError?.message || "Documento AML non trovato",
          });
          continue;
        }

        const { data: soggetto, error: soggettoError } = await supabaseAdmin
          .from("tbclienti")
        ...
        if (soggettoError || !soggetto?.id || !soggetto.email) {
          errori++;
          erroriScadenza++;
          dettagli.push({
            scadenza_id: riga.id,
            ok: false,
            messaggio: soggettoError?.message || "Soggetto AML o email non trovati",
          });
          continue;
        }
        ...
        if (!documentoAml.public_doc_enabled || !documentoAml.public_doc_token) {
          try {
            await sendRichiestaDocumentoRappresentante({
              documentoAmlId: documentoAml.id,
              studioId: documentoAml.studio_id,
              nomeDestinatario: soggetto.ragione_sociale || "Cliente",
              email: String(soggetto.email).trim(),
            ...
            dettagli.push({
              scadenza_id: riga.id,
              ok: true,
              messaggio: `Richiesta automatica documento inviata a ${soggetto.email}`,
            });
          } catch (e: any) {
            errori++;
            erroriScadenza++;
            dettagli.push({
              scadenza_id: riga.id,
              ok: false,
              messaggio: e?.message || "Errore invio richiesta documento AML",
            });
          }
        } else {
          saltati++;
          ...
          dettagli.push({
            scadenza_id: riga.id,
            ok: true,
            messaggio: `Link documento AML già attivo per ${soggetto.email}`,
          });
        }

        const prossimoAlert = calcolaProssimoAlert(riga, oggi);
        const now = new Date().toISOString();
        const update: any = { prossimo_alert_at: prossimoAlert, updated_at: now };

        if (inviatiScadenza > 0) {
          update.numero_alert_inviati = Number(riga.numero_alert_inviati || 0) + inviatiScadenza;
          update.ultimo_alert_inviato_at = now;
        }
      ...
      for (const destinatario of destinatari) {
        const destinatarioId = destinatario.utente_id;
        const destinatarioEmail = destinatario.email;
        const senderUserId =
          automaticSender?.senderUserId ||
          (destinatarioId && mittentiValidiSet.has(destinatarioId)
            ? destinatarioId
            : mittenteAlternativoId);

        const { data: logCreato, error: prenotazioneError } = await supabaseAdmin
          .from("tbscadenze_centrale_alert_log")
          .insert({
            studio_id: riga.studio_id,
            scadenza_id: riga.id,
            operatore_responsabile_id:
              riga.operatore_responsabile_id || destinatarioId || mittenteAlternativoId,
            destinatario_utente_id: destinatarioId,
            alert_numero: Number(riga.numero_alert_inviati || 0) + inviatiScadenza + 1,
            giorni_preavviso: Math.max(differenzaGiorni(riga.data_scadenza, oggi), 0),
            data_programmata: riga.prossimo_alert_at || adesso,
            canale: "email",
            destinatario_email: destinatarioEmail,
            esito: "in_lavorazione",
            tipo_alert: alert.tipoAlert,
            chiave_invio: alert.chiaveInvio,
          })
          .select("id")
          .maybeSingle();
        ...
        if (prenotazioneError) {
          if (prenotazioneError.code === "23505") {
            ...
            dettagli.push({
              scadenza_id: riga.id,
              ok: true,
              messaggio: `Alert già processato per ${destinatarioEmail}`,
            });
            continue;
          }
        ...
        if (!senderUserId) {
          ...
          await supabaseAdmin
            ...
            .update({
              esito: "errore",
              errore: "Nessun utente dello studio possiede un token Microsoft valido",
            })
            .eq("id", logCreato!.id);
          continue;
        }
        ...
        const nomeDestinatario =
          destinatario.tipo === "esterno"
            ? "Cliente"
            : [destinatario.nome, destinatario.cognome].filter(Boolean).join(" ") || destinatarioEmail;
        ...
        const risultatoInvio = await sendEmailServer({
          senderUserId,
          microsoftConnectionId,
          fromMailbox,
          to: destinatarioEmail,
          subject: alert.oggetto,
          html,
        });
        ...
        if (!risultatoInvio.success) {
          ...
          await supabaseAdmin
            ...
            .update({
              esito: "errore",
              errore: risultatoInvio.error || "Errore invio email",
            })
            .eq("id", logCreato!.id);
          ...
          dettagli.push({
            scadenza_id: riga.id,
            ok: false,
            messaggio: `${risultatoInvio.error || "Errore invio email"} - ${destinatarioEmail}`,
          });
          continue;
        }
        ...
        await supabaseAdmin
          .from("tbscadenze_centrale_alert_log")
          .update({ esito: "inviato", inviato_at: new Date().toISOString(), errore: null })
          .eq("id", logCreato!.id);
        ...
        dettagli.push({
          scadenza_id: riga.id,
          ok: true,
          messaggio: `Alert inviato a ${destinatarioEmail}`,
        });
      }
      ...
      dettagli.push({
        scadenza_id: riga.id,
        ok: erroriScadenza === 0,
        messaggio: `Destinatari: ${destinatari.length}; inviati: ${inviatiScadenza}; errori: ${erroriScadenza}; già processati: ${saltatiScadenza}`,
      });
    }

    return res.status(errori > 0 ? 207 : 200).json({
      ok: errori === 0,
      trovati: scadenze?.length || 0,
      inviati,
      saltati,
      errori,
      dettagli,
    ...
    console.error("Errore processore unico scadenze:", error);
    return res.status(500).json({
      ok: false,
      error: error?.message || "Errore processore unico delle scadenze",
    });
  }
}
