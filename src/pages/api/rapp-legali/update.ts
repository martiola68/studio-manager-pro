  ...
  try {
    ...
    if (!soggettoClienteId) {
      return res.status(400).json({
        ok: false,
        error:
          "ID anagrafica rappresentante mancante",
      });
    }
    ...
    if (soggettoLookupError) {
      return res.status(500).json({
        ok: false,
        error:
          soggettoLookupError.message,
      });
    }

    if (!soggettoEsistente?.id) {
    ...
    /*
     * =========================================================
     * 2. CONTROLLO CODICE FISCALE
     * =========================================================
     *
     * Evitiamo che il nuovo CF appartenga
     * a un'altra anagrafica dello stesso studio.
     */
    const {
    ...
    if (soggettoUpdateError) {
      return res.status(500).json({
        ok: false,
        error:
          soggettoUpdateError.message,
      });
    }

    /*
     * =========================================================
     * 4. RECUPERO DOCUMENTO AML
     * =========================================================
     *
     * Il documento viene cercato tramite:
     *
     * soggetto_cliente_id = tbclienti.id
     *
     * Non utilizziamo rapp_legali.
     */
    const {
      data: documentiAml,
    ...
    if (documentoAml?.id) {
      ...
      /*
       * =======================================================
       * 5B. DOCUMENTO NON ESISTENTE → INSERT
       * =======================================================
       *
       * La modifica dell'anagrafica deve funzionare
       * anche se per errore/migrazione il record AML
       * non esiste ancora.
       */
      const {
        data,
    ...
    /*
     * =========================================================
     * 6. SINCRONIZZAZIONE SCADENZA CENTRALE
     * =========================================================
     */
    ...
    /*
     * Cerchiamo l'eventuale scadenza
     * centrale collegata al documento AML.
     */
    const {
      data: scadenzeCentrali,
      error:
        scadenzaCentraleLookupError,
    } = await supabaseAdmin
      .from(
        "tbscadenze_centrale"
      )
    ...
    const scadenzaCentraleEsistente =
      scadenzeCentrali?.[0] ||
      null;
    ...
    if (!nuovaScadenzaDocumento) {
      ...
      if (
        ...
        /*
         * Il ciclo degli alert viene
         * azzerato solo se cambia
         * realmente la scadenza.
         */
        if (
          scadenzaDocumentoCambiata
        ) {
