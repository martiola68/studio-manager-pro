...
async function callInternal(path: string, method: "GET" | "POST" = "GET") {
  ...
  try {
    ...
    if (logError) console.error("Errore tbcron_log:", logError);
    return { path, method, ok: response.ok, status: response.status, durata_ms: durataMs, body: text };
  } catch (error: any) {
    const durataMs = Date.now() - startedAt;
    ...
    return { path, method, ok: false, status: 500, durata_ms: durataMs, body: errore };
  }
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const results = await Promise.all([
    callInternal(`/api/scadenze-centrale/processa-alert?secret=${SECRET}`),
    callInternal(`/api/scadenze/affitti/processa?secret=${SECRET}`),
    callInternal(`/api/scadenze/tipi/processa?secret=${SECRET}`),
    callInternal(`/api/cron/aml-fascicoli-alert`, "GET"),
    callInternal(`/api/presenze/sollecito-settimanale?secret=${SECRET}`),
    callInternal(`/api/presenze/report-giornaliero-email?secret=${SECRET}`),
    callInternal(`/api/revisione-controllo/followup-alert?secret=${SECRET}`, "POST"),
