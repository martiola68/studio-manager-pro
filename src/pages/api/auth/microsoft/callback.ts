...
export default async function handler(
  ...
  try {
    ...
    console.log("🔐 [Microsoft Callback] Requesting token with client_credentials...");
    console.log("🔐 [Microsoft Callback] User ID:", userId);

    // Ottieni access token APP-ONLY (client credentials)
    ...
    // Inserisci il nuovo token (nota: nessun refresh_token nel flusso client_credentials)
    const { error: insertError } = await supabase
      .from("tbmicrosoft_tokens")
      .insert({
        user_id: userId,
        access_token: accessToken,
        refresh_token: null, // Non disponibile nel flusso client_credentials
