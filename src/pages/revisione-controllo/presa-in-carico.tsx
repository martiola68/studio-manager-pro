...
export default function PresaInCaricoRevisione() {
  ...
  async function caricaBozzaEsistente() {
    ...
    const { data: documenti, error: dError } = await supabase.from("tbrevisione_preincarico_documenti").select("tipo,contenuto,stato").eq("preincarico_id", data.id).in("tipo", ["ATTESTAZIONE_INDIPENDENZA", "DICHIARAZIONE_TRASPARENZA"]);
  ...
  return (
    <main style={{ maxWidth: 1280, margin: "0 auto", padding: "28px 24px 60px", fontFamily: "Inter,Arial,sans-serif", color: "#0f172a" }}>
      ...
      {step > 6 && <section style={card}><h2 style={{ marginTop: 0 }}>Step {step} — {steps[step - 1]}</h2><p style={{ color: "#64748b" }}>Modulo predisposto nel workflow. Il documento dello step sarà inserito nel blocco dedicato.</p></section>}
