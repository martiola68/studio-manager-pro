...
type Studio = StudioBase & { microsoft_connection_id_tenant2?: string | null; ragione_sociale_tenant2?: string | null; email_tenant2?: string | null };
type UtenteRow = Database["public"]["Tables"]["tbutenti"]["Row"];
type HeaderUser = Pick<UtenteRow, "id" | "nome" | "cognome" | "email" | "tipo_utente" | "studio_id" | "settore" | "ruolo_operatore_id" | "attivo" | "microsoft_connection_id">;
...
export default function Header({ onMenuToggle, title }: HeaderProps) {
  ...
  const getStudioLabelForUser = (utente: HeaderUser | null, studioData: Studio | null) => {
    ...
    if (!utente?.microsoft_connection_id) return studioData.ragione_sociale || "";
    const isTenantSecondario = !!studioData.microsoft_connection_id_tenant2 && utente.microsoft_connection_id === studioData.microsoft_connection_id_tenant2;
    return isTenantSecondario ? (studioData.ragione_sociale_tenant2 || studioData.ragione_sociale || "") : (studioData.ragione_sociale || "");
  };
  const displayedStudioName = useMemo(() => getStudioLabelForUser(currentUser, studio), [currentUser, studio]);
  ...
  const loadUserAndStudio = async () => {
    try {
      ...
      if (!session?.user?.email) { setCurrentUser(null); setStudio(null); return; }
      const { data: utente, error: utenteError } = await supabase.from("tbutenti").select("id, nome, cognome, email, tipo_utente, studio_id, settore, ruolo_operatore_id, attivo, microsoft_connection_id").eq("email", session.user.email).maybeSingle();
      if (utenteError) { console.warn("Impossibile caricare utente:", utenteError.message); setCurrentUser(null); } else setCurrentUser((utente ?? null) as HeaderUser | null);
      const studioData = await studioService.getStudio();
      setStudio((studioData ?? null) as Studio | null);
    } catch (e) { console.error("Errore caricamento dati header:", e); setCurrentUser(null); setStudio(null); }
  };
  ...
  useEffect(() => { void controllaVersioneApplicazione(); const interval = setInterval(() => void controllaVersioneApplicazione(), 60000); return () => clearInterval(interval); }, []);
  useEffect(() => {
    const supabase = getSupabaseClient();
    void loadUserAndStudio();
    const { data: authListener } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === "SIGNED_OUT" || !session) { setCurrentUser(null); setStudio(null); }
      else if (event === "SIGNED_IN" || event === "TOKEN_REFRESHED") void loadUserAndStudio();
    });
  ...
  return <header className="relative border-b border-cyan-200/30 bg-[linear-gradient(110deg,#0b4f7d_0%,#0d6f9f_58%,#1688b7_100%)] shadow-sm">
    <div className="h-[64px] px-4 md:px-6 py-0"><div className="flex h-full items-center justify-between gap-3">
      <div className="flex h-full items-center gap-3 min-w-0"><Button variant="ghost" size="icon" className="lg:hidden shrink-0" onClick={onMenuToggle}><Menu className="h-6 w-6" /></Button><img src="/logo-elma.png" alt="Studio Manager Pro" className="h-full w-auto max-w-none object-contain shrink-0" /><div className="min-w-0 hidden sm:block"><h1 className="text-lg font-bold text-white leading-tight">{title || "Studio Manager Pro"}</h1><p className="text-xs text-cyan-100/80">Sistema Gestionale Integrato</p></div><a href="https://studiomanagerpro.it" target="_blank" rel="noopener noreferrer" className="hidden sm:inline-flex h-8 shrink-0 items-center gap-1.5 rounded-md border border-white/30 bg-white/10 px-3 text-xs font-semibold text-white transition-colors hover:bg-white/20" title="Vai al sito web"><ExternalLink className="h-3.5 w-3.5" />Sito web</a></div>
