...
const guardFile = "src/components/security/SensitiveDataMasterGuard.tsx";
...
function clearMasterSessionKeys() {
  ...
  for (let i = 0; i < sessionStorage.length; i += 1) {
    ...
    if (key && (key.startsWith("smp_master_unlocked:") || key.startsWith("smp_master_last_activity:"))) {
      keys.push(key);
    }
  }
...
export function SensitiveDataMasterGuard({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const loadProtection = async () => {
      ...
      try {
        ...
        const enabled = studio?.protezione_attiva === true && Boolean(studio?.master_password_hash);
        ...
        const stillValid = wasUnlocked && lastActivity > 0 && Date.now() - lastActivity <= MASTER_UNLOCK_TIMEOUT;
        ...
        console.error("[SensitiveDataMasterGuard] Verifica protezione non riuscita:", error);
        // Fail-open only on configuration lookup errors so an infrastructure/RLS issue
        // cannot block existing Studio Manager Pro workflows.
        if (!cancelled) {
          setProtectionEnabled(false);
          setUnlocked(true);
        }
  ...
  useEffect(() => {
    if (!protectedPath || !protectionEnabled || !unlocked || !studioId || !userId) return;
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "La protezione resta attiva fino a 15 minuti di inattività.",
      });
    } catch (error) {
    ...
    <>
      <div className="flex min-h-[360px] items-center justify-center px-4 py-10">
        <div className="w-full max-w-xl rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm">
          ...
          <h1 className="text-2xl font-bold text-slate-900">Dati sensibili protetti</h1>
          <p className="mt-3 text-sm leading-6 text-slate-600">
            Inserisci la Master Password dello studio per aprire quest'area. La pagina resta invariata dopo lo sblocco.
          </p>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              La password viene verificata solo per autorizzare l'accesso a questa sessione e non viene memorizzata.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <Input
              ...
              onKeyDown={(event) => {
                if (event.key === "Enter" && !unlocking) void handleUnlock();
              }}
            />
          </div>
          <DialogFooter>
...
fs.mkdirSync("src/components/security", { recursive: true });
fs.writeFileSync(guardFile, guardContent, "utf8");

{
  const file = "src/pages/_app.tsx";
  let s = fs.readFileSync(file, "utf8");

  if (!s.includes('import { SensitiveDataMasterGuard } from "@/components/security/SensitiveDataMasterGuard";')) {
    s = replaceOnce(
      s,
  ...
  s = replaceOnce(
    ...
    '<ModuleAccessGuard><SensitiveDataMasterGuard>{pageContent}</SensitiveDataMasterGuard></ModuleAccessGuard>',
    "_app protected page wrapper"
  );

  fs.writeFileSync(file, s, "utf8");
}

console.log("Master password completion v1 patch applied");
